...
function findRoutes(routeBaseDir) {
  return fs
    .readdirAsync(routeBaseDir)
    .then(files => {
      return Promise.mapSeries(files, filename => {
        //const isMbtilesFile = filename.match(/\.mbtiles$/i)
        const file = path.resolve(routeBaseDir, filename)
    ...
    .then(routes => _.reduce(routes, (result, route) => {
      result[route.identifier] = route
      return result
    }, {}))
}
...
/*function openMbtilesFile(file, filename) {
  return new Promise((resolve, reject) => {
    ...
    return {
      ...
      type: 'tilelayer',
      tilemapUrl: `${apiRoutePrefix}/routes/${identifier}/{z}/{x}/{y}`,
      scale: metadata.scale || '250000'
    }
  }).catch(e => {
    console.error(`Error loading route ${file}`, e.message)
    return null
  })
}*/
...
/*function directoryToMapInfo(file, identifier) {
  ...
  return loadInfo()
    .then(info => {
      if (info) {
        if (!info.format) {
          console.error(`Missing format metadata for route ${identifier}`)
          return null
        }
        return _.merge(info, {
          identifier,
    ...
    .catch(e => {
      console.error(`Error getting routes from ${file}`, e.message)
      return undefined
    })
}*/
